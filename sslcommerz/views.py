...
def get_razorpay_client():
    """Lazy load razorpay client to avoid import errors"""
    try:
        import razorpay
        return razorpay.Client(auth=(settings.RAZORPAY_KEY_ID, settings.RAZORPAY_KEY_SECRET))
    except (ImportError, AttributeError) as e:
        raise Exception(f"Razorpay SDK not properly installed: {str(e)}")
...
@csrf_exempt
def razorpay_payment_request(request, pk, id):
    ...
    try:
        ...
        amount = int((appointment.doctor.consultation_fee + appointment.doctor.report_fee) * 100)
        ...
        # Save appointment transaction ID
        appointment.transaction_id = order_id
        appointment.save()
...
@csrf_exempt
def razorpay_payment_request_medicine(request, pk, id):
    """Create Razorpay order for medicine payment"""
    try:
        patient = Patient.objects.get(patient_id=pk)
        order = Order.objects.get(id=id)
        
        invoice_number = generate_random_invoice()
        amount = int(order.final_bill() * 100)
...
@csrf_exempt
def razorpay_payment_request_test(request, pk, id, pk2):
    ...
    try:
        ...
        invoice_number = generate_random_invoice()
        amount = int(test_order.final_bill() * 100)
        ...
        # Save test order transaction ID
        test_order.trans_ID = order_id
        test_order.save()
        
        # Create payment record
        payment = Payment()
        payment.patient = patient
        payment.test_order = test_order
...
def verify_payment(request):
    ...
    try:
        ...
        # Get client and verify signature
        razorpay_client = get_razorpay_client()
        ...
        payment.val_transaction_id = razorpay_payment_id
        payment.currency_amount = payment_details.get('amount', 0) / 100
        payment.status = 'captured'
        payment.transaction_date = str(payment_details.get('created_at', ''))
        payment.currency = payment_details.get('currency', 'INR')
        payment.card_type = payment_details.get('method', '')
        payment.save()
        
        # Handle appointment payment
        if payment_type == "appointment":
            appointment = Appointment.objects.get(transaction_id=razorpay_order_id)
            appointment.payment_status = "VALID"
            appointment.save()
        
        # Handle test payment
        elif payment_type == "test":
            test_order = testOrder.objects.get(trans_ID=razorpay_order_id)
            test_order.payment_status = "VALID"
            test_order.save()
            
            # Update prescription tests
            prescription = payment.prescription
            testcarts = testCart.objects.filter(testorder__trans_ID=razorpay_order_id)
            
            for cart in testcarts:
                test_id = cart.item.test_info_id
                Prescription_test.objects.filter(prescription=prescription, test_info_id=test_id).update(test_info_pay_status="Paid")
        ...
        # Handle pharmacy payment
            ...
            order = Order.objects.get(trans_ID=razorpay_order_id)
...
@csrf_exempt
def razorpay_payment_fail(request):
    """Payment failed page"""
...
def razorpay_payment_request(request, pk, id):
    ...
    invoice_number = generate_random_invoice()
    amount = int((appointment.doctor.consultation_fee + appointment.doctor.report_fee) * 100)  # Amount in paise
...
@csrf_exempt
def razorpay_payment_request_medicine(request, pk, id):
    # Payment Request for medicine order
    
    patient = Patient.objects.get(patient_id=pk)
    order = Order.objects.get(id=id)
    
    invoice_number = generate_random_invoice()
    amount = int(order.final_bill() * 100)  # Amount in paise
...
@csrf_exempt
def razorpay_payment_request_test(request, pk, id, pk2):
    # Payment Request for test order
    ...
    invoice_number = generate_random_invoice()
    amount = int(test_order.final_bill() * 100)  # Amount in paise
        ...
        try:
            ...
            if payment_type == "appointment":
                ...
                values = {
                    ...
                    "tran_id": razorpay_payment_id,
                    "currency_amount": payment_details.get('amount', 0) / 100,
                    "transaction_date": payment_details.get('created_at', ''),
                }
                ...
                # Update prescription tests as paid
                prescription = payment.prescription
                ob = testCart.objects.filter(testorder__trans_ID=razorpay_order_id)
